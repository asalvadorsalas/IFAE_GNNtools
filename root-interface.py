...
def eval_append_ntuple(ntuple, tree_name, outpath, model, global_features, global_scale, node_scale):
    ...
    variables_to_read=['jet_pt', 'jet_phi', 'jet_eta', 'jet_e', 'jet_tagWeightBin_DL1r_Continuous','jet_qg_BDT',
               'mu_pt','mu_phi', 'mu_eta', 'mu_e', 'el_pt','el_phi', 'el_eta','el_e', 'nJets', 'met_met', 'met_phi',
               'eventNumber','runNumber', 'mcChannelNumber','nBTags_DL1r_70']+global_features
    ...
    graphs, booking = CreateTorchGraphs(data, global_features, global_scale, node_scale, remove_phi=True, include_qg_bdt=True)
...
# Parse args
import argparse
parser = argparse.ArgumentParser(description='Evaluate ntuples with GNN model and append classification scores to cloned ntuples.')
...
def event2ptgeo(event, global_vars, global_scale, node_scale, label, pseudo_mass):
    ...
    edge_coordinate_tuples=list(permutations(range(n_nodes), 2)) # Use permutations for directed graph, combinations for undirected
    ...
    for i in range(n_edges):
        ...
        delta_phi=np.arctan2(np.sin(node_scale[1]*(x[row][1]-x[col][1])), np.cos(node_scale[1]*(x[row][1]-x[col][1])))/node_scale[1]
...
def tree2graphs_eval(tree, global_vars, global_scale, node_scale):
    ...
    for event in tree:
        ...
        if i%100==0 or i==n_entries:
            ...
            seconds_remaining = graphs_remaining/graphs_per_second
            print('\rCreating graphs. {}/{} complete. Time elapsed: {:.5}s,   Estimated time remaining: {:.5}s'.format(i, n_entries, elapsed, seconds_remaining), end='')
        graph=event2ptgeo(event, global_vars, global_scale, node_scale, -999, -999) # Will define pseudo-mass during evaluation later, label doesn't matter
        graphs.append(graph)
    print('\nDone!')
    return graphs
